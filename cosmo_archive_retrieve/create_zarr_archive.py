...
def process_tar_file(leadtime, tar_file_paths, num_tot_leadtimes, hour_start, tmpdir):
    ...
    with tempfile.TemporaryDirectory(prefix='/scratch/cosuna/') as tarextract_dir:
        ...
        logger.info(f"Copying from archive ({tarextract_dir}) to {os.path.join(tarextract_dir, Path(ana_file).name)}")
        ...
        for hour in range(first_leadtime_of_day, 24):
            ...
            group_ana_files.append( os.path.join(tarextract_dir, "ANA", f"{tdate.strftime('%Y%m%d')}/det/laf{tdate.strftime('%Y%m%d%H')}"))
            group_fg_files.append( os.path.join(tarextract_dir, "FG", f"{tdate.strftime('%Y%m%d')}/det/lff{tdate.strftime('%Y%m%d%H')}"))
            
        for index, (ana_full_path, fg_full_path) in enumerate(zip(group_ana_files, group_fg_files)):
...
def load_data(config: dict) -> None:
    ...
    if zarr_checkpoint:
        checkpoint_hours_from_start = int((zarr_checkpoint - first_date_avail).total_seconds() / 3600)
