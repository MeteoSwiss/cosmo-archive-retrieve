...
def load_data(config: dict) -> None:
    ...
    files_list=[]
    data_path = os.path.join(config["data_path"], "ANA"+config["train_years"][0], "det")
    ...
    fg_files_list=[]
    fg_regex = re.compile("lff.*")
    data_path = os.path.join(config["data_path"], "FG"+config["train_years"][0], "det")
    for root, _, files in os.walk(data_path):
            for filename in files:
                if fg_regex.match(filename):
                    fg_files_list.append(os.path.join(root, filename))
...
def adapt_coords_to_zarr(
    ...
    var = var.drop_vars(["ref_time", "valid_time"])
...
def process_ana_file(full_path: str, config):
    ...
    try:
        ...
        for name, var in ds.items():
            ...
            if name == "HHL" or name == "W":
                ...
                for coord in ("valid_time", "ref_time"):
                    old_coords[coord] = var.coords[coord]
                z_coord = var.coords['z']

                var = destagger(var, "z")
                var = var.assign_coords(old_coords)
                var = var.assign_coords({'z': z_coord[:-1]})
        ...
        pdset["FI"] = pdset["FI"].assign_attrs(pdset["HFL"].attrs)
        pdset["FI"] = pdset["FI"].assign_coords(pdset["HFL"].coords)
        pdset["FI"].attrs |= metadata.override( pdset["FI"].message, shortName="FI")

        # P0FL should be a time invariant
#        pdset["P0FL"] = (pdset["P"] - pdset["PP"]).squeeze(dim="time")
#        pdset["P0FL"] = pdset["P0FL"].assign_attrs(pdset["HFL"].attrs)
#        pdset["P0FL"] = pdset["P0FL"].assign_coords(pdset["HFL"].coords)
#        pdset["P0FL"].attrs |= metadata.override( pdset["P0FL"].message, shortName="P0FL")

        pdset["RELHUM"] = relhum(
            pdset["QV"], pdset["T"], pdset["P"], clipping=True, phase="water"
        )
        pdset["RELHUM"].attrs |= metadata.override( pdset["RELHUM"].message, shortName="RELHUM")
...
#                continue
            with data_source.cosmo_grib_defs():
                pathname=os.path.join(config["out_path"], os.path.basename(full_path))
...
def process_fg_file(full_path: str, config) -> xr.Dataset:
